...
Radio is the ID used to identify your radio in DMR transmissions. Radio ID is a key part of the DMR standard, and all brands of DMR radio will transmit and decode your radio ID from one radio to another. You can use any number you want as a radio ID; however certain networks such as Brandmeister may require a radio ID assigned by a central body, such as [radioid.net]
 ...
 New Radio IDs cannot be set from the front panel, they must be set by the CPS. Once set, you can change the radio ID used by a particular channel in the CPS or from the radio's front panel. For radios running firmware version 3.04 or earlier, the radio ID is set per channel, and cannot be set globally. Firmware version 3.05 and later introduced "Master Radio ID." Master radio ID can be chosen from the list of radio IDs included in the codeplug from the front panel, new radio IDs still cannot be added from the front panel. If a channel is set to use master radio ID, it will automatically be updated when the master radio ID is changed. If a channel is set to use a specific radio ID, it will not be affected by changing the master radio ID.
